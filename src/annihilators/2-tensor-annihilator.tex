...
\begin{frame}[fragile]
   \begin{tikzpicture}
            ...
            \draw[thick,->] (T) -- (TY) node[midway,above] {$I-Y$};
            \draw[thick,->] (TY) edge[out=-45, in=45, looseness=4] (TY);

            \draw[thick,->] (T) -- (XT) node[midway,left] {$I-X$};
            \draw[thick,->] (XT) edge[out=-45, in=-135, looseness=4] (XT);


            \draw[thick,->] (XT) -- (XTY) node[midway,above] {$I-Y$};
            \draw[thick,->] (TY) -- (XTY) node[midway,left] {$I-X$};

            \draw[thick,->] (XTY) edge[out=-45, in=45, looseness=4] (XTY);
            \draw[thick,->] (XTY) edge[out=-45, in=-135, looseness=4] (XTY);
        \end{tikzpicture}
\end{frame}
